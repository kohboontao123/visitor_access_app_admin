...
class _EditResidentScreenState extends State<EditResidentScreen> {
  ...
  String imgPath ="";
  var img = Image.network(ResidentClass.userImage.toString());
  late var gender;
  ...
  void initState(){
    super.initState();
    _nameTextController=TextEditingController(text:ResidentClass.name);
    _idTextController=TextEditingController(text:ResidentClass.icNumber);
    _addressTextController=TextEditingController(text:ResidentClass.address);
    _phoneNumberTextController=TextEditingController(text:ResidentClass.phoneNumber);
    _emailTextController=TextEditingController(text:ResidentClass.email);
    gender=ResidentClass.gender;
    ...
    final btnDeactive = MaterialButton(
      ...
      child: Padding(
        ...
        child: Container(
            padding: EdgeInsets.all(10.0),
            decoration: BoxDecoration(
              color:ResidentClass.status=="active"? Colors.red: Colors.green,
              borderRadius: BorderRadius.circular(12),
            ),
            child: Center(
              child: _btnActiveIsLoading
                ...
                children: [
                  ...
                  :ResidentClass.status=="active"?Text(
                'Deactivate',
                style: TextStyle(
                    color: Colors.white,
                    fontWeight: FontWeight.bold,
  ...
  update() async {
    ...
    residentModel.email=ResidentClass.email;
    residentModel.uid=ResidentClass.uid;
    ...
    collection
        .doc(ResidentClass.uid) // <-- Doc ID where data should be updated.
        .update(residentModel.toMap()) // <-- Nested value
        .then((_) => Fluttertoast.showToast(msg: "The resident's profile has been successfully updated"))
  ...
  activate() async {
    try{
      var name = ResidentClass.name;
      if(ResidentClass.status=="active"){
        var collection = FirebaseFirestore.instance.collection('resident');
        collection
            .doc(ResidentClass.uid) // <-- Doc ID where data should be updated.
      ...
      Navigator.pushAndRemoveUntil((context), MaterialPageRoute(builder: (context) => ManageResidentScreen()), (route) => false);
