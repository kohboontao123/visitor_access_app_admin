...
import 'package:visitor_access_app_admin/model/visitor_model.dart';
...
class _EditVisitorScreenState extends State<EditVisitorScreen> {
  ...
  String imgPath ="";
  var img = Image.network(VisitorClass.userImage.toString());
  late var gender;
  ...
  void initState(){
    super.initState();
    _nameTextController=TextEditingController(text:VisitorClass.name);
    _idTextController=TextEditingController(text:VisitorClass.icNumber);
    _addressTextController=TextEditingController(text:VisitorClass.address);
    _phoneNumberTextController=TextEditingController(text:VisitorClass.phoneNumber);
    _emailTextController=TextEditingController(text:VisitorClass.email);
    gender=VisitorClass.gender;
    ...
    final btnDeactive = MaterialButton(
      ...
      child: Padding(
        ...
        child: Container(
            padding: EdgeInsets.all(10.0),
            decoration: BoxDecoration(
              color:VisitorClass.status=="active"? Colors.red: Colors.green,
              borderRadius: BorderRadius.circular(12),
            ),
            child: Center(
              child: _btnActiveIsLoading
                ...
                children: [
                  ...
                  :VisitorClass.status=="active"?Text(
                'Deactivate',
                style: TextStyle(
                    color: Colors.white,
                    fontWeight: FontWeight.bold,
                ...
                child:Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    image: DecorationImage(
                      image: NetworkImage(VisitorClass.icImage.toString()),
                      fit: BoxFit.cover
                    )
                  ),
                ) ,
  ...
  update() async {
    ...
    visitorModel.name=_nameTextController.text;
    visitorModel.address=_addressTextController.text;
    visitorModel.gender= gender;
    visitorModel.phoneNumber=_phoneNumberTextController.text;
    visitorModel.icNumber=_idTextController.text;
    visitorModel.status = 'active';
    var collection = FirebaseFirestore.instance.collection('visitor');
    collection
        .doc(VisitorClass.uid) // <-- Doc ID where data should be updated.
        .update(visitorModel.update()) // <-- Nested value
        .then((_) => Fluttertoast.showToast(msg: "The profile has been successfully updated"))
  ...
  activate() async {
    try{
      var name = VisitorClass.name;
      if(VisitorClass.status=="active"){
        var collection = FirebaseFirestore.instance.collection('visitor');
        collection
            .doc(VisitorClass.uid) // <-- Doc ID where data should be updated.
      ...
      Navigator.pushAndRemoveUntil((context), MaterialPageRoute(builder: (context) => ManageVisitorScreen()), (route) => false);
